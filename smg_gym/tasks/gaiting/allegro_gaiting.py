...
class AllegroGaiting(BaseAllegro):
    ...
    def __init__(
        ...
        super(AllegroGaiting, self).__init__(
            cfg,
            sim_device,
            graphics_device_id,
            headless
    ...
    def check_mean(self,successes_tensor,c_successes_tensor ,progress_buf):
        # mean_value = torch.mean(tensor)
        # Calculate success rate and other metrics
        success_rate = torch.mean(successes_tensor / (c_successes_tensor + 1))
    
        # Define adaptive thresholds
        # success_rate_threshold = 0.7  # Adjust as needed
        # Define adaptive thresholds
        success_rate_upper_threshold = max(success_rate+0.1, 0.5)  # Adjust as needed (upper threshold for increase)
        success_rate_lower_threshold = min(success_rate-0.1, 0.3)  # Adjust as needed (lower threshold for decrease)


        # Check if success rate is below the threshold
        if self.count>=200:
            self.count=0
            print(success_rate, success_rate_lower_threshold)
            if self.last_success <= success_rate and success_rate <= success_rate_upper_threshold and self.cfg["env"]["reach_goal_bonus"] <500:
                # Modify environment parameters
                
                self.cfg["env"]["reach_goal_bonus"] = max(round(self.cfg["env"]["reach_goal_bonus"]* 1.2,2) ,1) # Increase the reach goal bonus
                self.cfg["env"]["drop_obj_penalty"] =  max(round(self.cfg["env"]["drop_obj_penalty"]* 0.8,2),0)  # Decrease the drop object penalty
                print("Up: ", self.count)
                print(self.cfg["env"]["reach_goal_bonus"],self.cfg["env"]["drop_obj_penalty"] )
            elif success_rate > success_rate_lower_threshold or self.cfg["env"]["reach_goal_bonus"] >1: 
                print("Down: ",self.count)
                self.cfg["env"]["reach_goal_bonus"] = max(round(self.cfg["env"]["reach_goal_bonus"]* 0.8,2),1)# Increase the reach goal bonus
                self.cfg["env"]["drop_obj_penalty"] = max(round(self.cfg["env"]["drop_obj_penalty"]* 1.2,2),0)   # Decrease the drop object penalty
                print(self.cfg["env"]["reach_goal_bonus"],self.cfg["env"]["drop_obj_penalty"] )
            
        self.last_success=success_rate
    ...
    def compute_reward_and_termination(self):
        ...
        # self.check_mean(self.successes,self.consecutive_successes,self.progress_buf)
        # self.count+=1
